# 资源中心 PostgreSQL + Milvus 双数据库技术方案

> 状态：拟议目标架构，尚未成为当前运行链路。
>
> 适用范围：仅资源中心的文档知识检索、语义搜索和 RAG。其他业务模块继续只依赖 PostgreSQL、Redis、对象存储及既有应用接口。
>
> 设计基线：PostgreSQL 是业务事实与授权事实的唯一来源；Milvus 只保存向量及检索所需的最小标量索引，不保存业务权限真相。
...
2. Milvus 保存分片向量和最小关联字段。向量主键使用确定性 `vector_id`，通过 `tenant_id`、`knowledge_base_id`、`resource_id`、`document_id`、`document_version_id`、`chunk_id` 与 PostgreSQL 关联。
3. 资源中心应用层拥有索引和检索用例；Milvus SDK 只出现在 Milvus adapter 中。AI 会话模块只调用 `KnowledgeRetriever` 领域接口，不能直接调用 Milvus。
4. 入库使用 PostgreSQL 事务 Outbox、持久化任务、独立 worker 和确定性 upsert，采用最终一致性，不做跨库分布式事务。
5. 发布采用“新版本完整建索引后再切换 active generation”的蓝绿索引方式。下线、禁用和删除先在 PostgreSQL 生效，再异步清理 Milvus；检索返回前必须再次用 PostgreSQL 批量鉴权和核对状态。
6. Collection 默认按“检索场景 + 向量模型族/维度”共享，`tenant_id` 作为 partition key 或必选标量过滤条件。禁止默认按用户、租户或知识库无限创建 Collection。
7. 混合检索的首选实现是 PostgreSQL 全文检索与 Milvus 向量检索并行，在资源中心检索协调器中使用 RRF 融合。这样不需要在 Milvus 保存分片原文，保持权责边界。未来若启用 Milvus BM25，必须另立 ADR，因为这会扩展 Milvus 的职责。
8. 开发和集成测试使用固定版本的 Milvus Standalone Docker Compose；生产要求高可用时使用 Kubernetes 上的 Milvus Distributed，或选择托管服务。Milvus Lite 仅适合本地实验，不作为 Go 后端的标准依赖。
...
- 不把 Milvus 变成资源、用户、权限或审计数据库。
- 不允许其他业务模块直接依赖 Milvus SDK、Collection 名称或向量字段。
- 不在第一阶段拆出一个对外公开的微服务。
- 不在 PostgreSQL 与 Milvus 之间实现 XA/2PC。
- 不让 pgvector 与 Milvus 同时成为资源中心向量的双重事实源。
...
flowchart LR
    ...
    JOB --> PARSER
    JOB --> EMB
    JOB -->|upsert / delete / query verify| MV
    JOB -->|更新任务与索引清单| PG
...
| Milvus | 向量、最小检索标量、ANN 索引、相似度召回 | 用户权限真相、发布真相、分片原文、事务审计 |
| 资源中心服务 | 资源和文档用例、发布规则、任务创建、状态展示 | 直接拼装 Milvus 供应商协议 |
...
| AI 会话服务 | 对话状态、检索调用、Prompt 编排、SSE 和答案持久化 | 直接访问 Milvus |
...
即使拆分，资源中心仍是数据所有者，检索服务使用内部鉴权接口，不对其他模块暴露 Milvus 细节。

### 3.4 与当前代码库的接入位置

| 现有位置 | 建议变化 |
|---|---|
| `backend/internal/application/resource` | 继续拥有资源、文档版本、发布、下线和索引任务用例；新增端口，不引入 Milvus SDK |
...
| `backend/internal/adapter/milvus` | 新增 Milvus schema、路由、upsert、delete、search 和 verify adapter |
...
- `embedding_models` 已存在，新增不可变版本表或版本字段时要迁移现有记录，不能删除重建。
...
Milvus 可以删除并从 PostgreSQL、对象存储和 Embedding 模型重新构建。反向从 Milvus 恢复业务数据是不允许的。
...
- `chunk_id` 在分片创建时生成并写入 PostgreSQL，不能使用 Milvus 自增 ID。
- `vector_id` 使用确定性字符串，例如：

```text
vector_id = UUIDv5(namespace, tenant_id | chunk_id | embedding_model_version_id)
```

- 一个分片在模型迁移期间可以对应多个 `vector_id`。
...
## 6. Milvus 数据模型与 Collection 策略

### 6.1 推荐 Collection Schema

Collection 示例：`resource_chunks_dense_v1`。

| 字段 | 类型 | 用途 |
|---|---|---|
| `vector_id` | VARCHAR primary key, auto ID false | 确定性幂等键 |
| `tenant_id` | VARCHAR, partition key | 共享 Collection 的强制租户过滤和数据分组 |
| `knowledge_base_id` | VARCHAR | 知识库过滤 |
| `resource_id` | VARCHAR | 资源回查和删除 |
| `document_id` | VARCHAR | 文档回查 |
| `document_version_id` | VARCHAR | 版本隔离 |
| `chunk_id` | VARCHAR | PostgreSQL 原文回查 |
| `embedding_model_version_id` | VARCHAR | 模型版本隔离 |
| `index_generation` | INT64 | 蓝绿代际过滤 |
| `chunk_hash` | VARCHAR | 对账 |
| `language` | VARCHAR | 可选语言过滤 |
| `visibility_class` | VARCHAR | 可选的粗粒度可见性投影 |
| `projected_state` | VARCHAR | 可重建的发布/删除投影 |
| `updated_at_epoch_ms` | INT64 | 对账和诊断 |
| `embedding` | FLOAT_VECTOR | 向量，维度由 Collection 固定 |
...
| 方案 | 优点 | 缺点 | 适用性 |
|---|---|---|---|
| 每租户一个 Collection | 物理边界直观、单租户迁移方便 | Collection 爆炸、索引和加载开销高、运维复杂 | 少量超大或强监管租户 |
| 每知识库一个 Collection | 生命周期独立 | 知识库多时控制面膨胀，跨库搜索困难 | 少量超大型知识库 |
| 全部共用一个 Collection | 管理简单、资源利用率高 | 必须严格过滤；不同维度不能共存 | 同模型、同场景、多租户主路径 |
| 按业务场景划分 | 参数和 SLO 可独立调优 | Collection 数增加，需要路由表 | RAG、图片、代码等检索特征明显不同 |

推荐采用“场景 + 模型族/维度”的共享 Collection：

```text
resource_chunks_dense_<model_family>_<schema_version>
```

规则如下：

1. 同一 Collection 的向量维度、距离度量和索引配置固定。
2. 租户使用 `tenant_id` partition key 或强制标量条件，知识库使用标量过滤。
3. 模型迁移创建新 Collection 或新代际，双读评估后切换，不能直接覆盖旧向量。
4. 超大或监管租户通过 PostgreSQL 中的 placement 配置路由到专属 Collection/Database。
5. 不允许请求方提交 Collection 名称；由服务端根据 scenario、model version 和 placement 决定。
...
- Milvus 写入使用确定性 `vector_id` 和 upsert。
- 失败按错误类别重试；永久错误进入 `DEAD`，只保存脱敏错误码和摘要。
- 定时对账校验 PostgreSQL manifest 与 Milvus 的 ID、数量、generation 和 hash。
...
2. 使用新的 `index_generation` 完成解析、切片、Embedding 和 Milvus upsert。
...
6. 只有标题、标签等非内容字段变化时，不重新向量化；更新 PostgreSQL，并按需刷新 Milvus 的最小过滤投影。

### 7.4 删除、发布、下线和禁用

| 操作 | PostgreSQL 同步动作 | Milvus 异步动作 | 对检索的即时影响 |
|---|---|---|---|
...
| 故障 | 处理 |
|---|---|
| PostgreSQL 成功、Milvus 失败 | job 进入 RETRY_WAIT；版本不进入 READY/PUBLISHED；保留已完成分片和 Embedding 可重用结果 |
| Milvus 成功、PostgreSQL 状态更新失败 | 重试同一 job；确定性 upsert 不产生重复向量；对账发现向量后补写 manifest/状态 |
| 部分批次写入 Milvus | 以 batch checkpoint 续传或整版本幂等重放；版本仍不可激活 |
| Embedding 成功、worker 崩溃 | 若未持久化向量缓存则重算；若成本高可将加密临时结果存对象存储并设置短 TTL |
| 下线事件延迟 | PostgreSQL 最终鉴权阻断，Milvus 投影仅是性能过滤 |
...
- Milvus primary key：确定性 `vector_id`；写入统一 upsert。
...
2. 每日分区对账：按 tenant、knowledge base、version、generation 比较 manifest 数、Milvus 实体数、hash 抽样和 active generation。
3. 周期全量：离线导出 ID/hash 清单做排序合并，避免在线逐行查询。

发现不一致时：

- PostgreSQL 有 manifest、Milvus 无向量：生成 `REINDEX_MISSING`。
- Milvus 有向量、PostgreSQL 无 manifest：先按版本和保留策略确认，再生成 `DELETE_ORPHAN`。
...
- PostgreSQL 到 Milvus 的新增、更新和物理删除；
- Milvus 状态投影刷新；
- 旧 generation 清理；
- 对账和统计。

Milvus 搜索可使用 Session 或 Bounded Staleness 作为默认起点，但安全性不依赖其一致性级别。发布切换前应显式等待本 generation 写入可见并完成校验。
...
3. Milvus 召回：默认 COSINE，返回较大的候选集而不是直接返回最终 TopK。
4. 过滤：服务端强制加入 `tenant_id`、允许知识库、active generation、模型版本和可检索状态；客户端字段不能覆盖强制条件。
5. 混合检索：PostgreSQL 全文/词法索引与 Milvus 并行；数学公式、专有名词、课程编号和精确标题更依赖关键词，语义改写更依赖向量。中文不能假设 PostgreSQL 默认词典已有理想分词，MVP 应结合规范化标题、`pg_trgm`/n-gram 或经评审的中文分词扩展做质量验证。
...
| 阶段 | 起始值 | 调优目标 |
|---|---:|---|
| Milvus dense candidate | 80 | recall@50 |
...
### 8.4 PostgreSQL 与 Milvus 的分工

- Milvus 只返回 `chunk_id + score + 最小关联字段`。
...
权限只存 PostgreSQL。Milvus 最多保存低基数、可重建的粗粒度投影：
...
Milvus 不保存用户列表、部门列表、角色列表或 deny ACL。原因是 ACL 变更频繁、数组过滤会膨胀、跨库更新难以原子化，并且会把权限真相复制到不适合审计的系统。
...
| 策略 | 安全 | 准确性 | 性能 | 结论 |
|---|---|---|---|---|
| 先在 PG 枚举全部 resource ID，再传 Milvus | 高 | 高 | 权限集合大时表达式过长、计划和网络开销高 | 仅小范围知识库适用 |
| 先 Milvus 全局召回，再 PG 后过滤 | 不足 | 低，TopK 可能全被过滤 | 表面快，但需扩大候选且存在侧信道 | 禁止作为唯一策略 |
| PG 生成粗范围，Milvus 预过滤，PG 最终复核 | 高 | 高 | 可控、集合稳定 | 推荐 |
...
- Milvus 使用 partition key 或强制标量过滤；高监管租户可使用专属 Database/Collection 和独立凭据。
...
`embedding_model_versions` 必须固化 provider、模型名、revision、dimension、metric、tokenizer、normalization、最大 token、状态和创建时间。Collection 路由由模型版本决定，禁止只按模型展示名判断兼容性。
...
应用 DTO 只使用领域 ID、filter 和候选结果，不出现 Milvus Collection、partition、SDK option 或供应商错误类型。
...
| `IndexDocumentVersion` | version ID, generation, vector batch | Milvus upsert + PostgreSQL manifest |
...
| 503 | `VECTOR_SEARCH_UNAVAILABLE` | Milvus 不可用 | true |
| 503 | `PARSER_UNAVAILABLE` | 解析服务瞬态故障 | true |

所有错误沿用项目稳定的 `code`、`message`、request ID 和 Retry-After 语义。公开消息不能包含 Milvus 地址、Collection 名、SQL、对象 URI、provider 原文或凭据。
...
| 环境 | 推荐 | 说明 |
|---|---|---|
| 个人开发 | Milvus Standalone Compose profile | 与生产 API 行为更接近；按需启动 |
| 快速算法实验 | Milvus Lite | 仅本地小数据实验；Go 服务不直接嵌入 |
| 集成测试 | 固定版本 Standalone + 隔离卷 | 跑 schema、过滤、upsert、delete、重建契约 |
| 预生产 | 与生产同拓扑或缩小版 Distributed | 使用脱敏代表数据压测 |
| 生产小规模且可停机 | Standalone | 仅在明确接受单点和维护窗口时 |
| 生产高可用 | Kubernetes + Milvus Distributed 或托管服务 | 推荐路径 |

不要使用 floating `latest`。Milvus server、客户端、备份工具、Helm chart 和依赖组件都固定到经过集成验证的兼容版本。

### 12.2 依赖与拓扑

- etcd 保存 Milvus 元数据，生产使用奇数节点高可用集群、独立磁盘、定期快照和受限网络。
- 对象存储保存持久数据和索引文件。开发可使用 MinIO；生产优先使用已有高可用 S3 兼容对象存储或高可用 MinIO。
- 分布式版本的流式/WAL 组件以所选 Milvus release 的官方 chart 为准，不能自行删除 chart 依赖。
- Query、Data、Index、Proxy/Coordinator 等组件按官方分布式拓扑部署，至少跨故障域放置副本。
- Go API 只拥有查询凭据；worker 拥有写入和删除凭据；运维凭据独立保管。

### 12.3 高可用和扩展

- Proxy、Query Node、Data Node 和 Index Node 设置多副本、PodDisruptionBudget、反亲和和跨可用区调度。
- etcd 和对象存储的可用性必须高于 Milvus 服务本身。
- Query Node 按已加载数据量、QPS 和并发扩容；Index Node 按索引积压和构建时间扩容；Data Node 按写吞吐扩容。
- 使用 resource group/placement 隔离关键租户或场景，避免大批重建挤占在线查询。
- 重建任务设置并发和带宽上限，在线查询优先。
- Collection、partition 和 segment 数量保持受控，禁止按用户创建。
...
4. Milvus Backup 或托管服务快照。

Milvus 是可重建副本，但全量重建的 RTO 和 Embedding 成本可能很高，因此生产仍应备份。每次协调备份生成 `backup_epoch` 清单，记录 PostgreSQL LSN/时间、对象存储版本、Milvus backup ID、active generations 和模型版本。

恢复流程：

1. 恢复 PostgreSQL 和对象存储；
2. 恢复 Milvus 快照或部署空集群；
3. 对照 manifest 校验 Collection schema、generation、数量和抽样 hash；
4. 缺失数据按 generation 重建；
5. 只有鉴权、发布状态和索引校验通过后恢复 RAG 流量。

灾备演练至少每季度执行一次，分别验证“Milvus 快照恢复”和“从 PostgreSQL/对象存储全量重建”。

### 12.5 升级

- 先阅读目标版本兼容矩阵和 breaking changes，在预生产恢复备份并回放代表流量。
- 升级前停止 schema 变更和大规模重建，完成 PostgreSQL、etcd、对象存储和 Milvus 备份。
- 客户端先确认兼容范围，再按官方顺序滚动升级服务组件。
- 新索引参数通过新 Collection/generation 灰度，不能在唯一在线索引上直接试验。
- 保留回滚版本、旧 generation 和恢复手册；完成对账、延迟和召回验收后再清理。

### 12.6 网络和监控

- Milvus、etcd、对象存储管理端口只在私网/集群网络开放，不通过前端 Nginx 暴露。
- 启用 TLS、服务身份认证、NetworkPolicy、安全组和最小权限。
- 指标接入现有 Prometheus：查询 QPS、P50/P95/P99、错误率、loaded entity、segment、compaction、索引队列、insert/delete rate、内存、磁盘、对象存储/etcd 延迟。
- 应用指标：检索各阶段耗时、候选数、授权过滤率、空召回率、降级率、Embedding/Rerank token、job lag、retry/dead、对账差异。
- 告警至少覆盖可用性、P95/P99、内存水位、索引积压、etcd 延迟/容量、对象存储错误、dead job、对账漂移和备份失败。
...
FLOAT_VECTOR 每维按 4 字节估算。实际内存还包括索引图/聚类、标量、segment、缓存、运行时和副本；对象存储还包括 binlog、索引文件、备份和旧 generation。

规划初值：

- HNSW 单副本已加载内存先按原始向量的 2 至 3 倍估算，再用真实数据校准。
- 节点保留至少 30% 内存和磁盘余量。
- 双模型迁移期间向量、索引和 Embedding 成本接近翻倍。
- 两个 Query 副本意味着加载内存近似按副本倍增。
...
HNSW 单副本先按约 9.2 至 13.7 GiB 估算；两个加载副本约 18.4 至 27.4 GiB，再加进程、segment、标量和 30% 余量，Query 层可从 32 至 48 GiB 总内存开始压测。该数字是预算起点，不能替代真实索引构建后的监控值。
...
这是同区域网络下的工程目标，不是 Milvus 保证值。LLM 首 token 和完整生成时间单独计量。

### 13.5 索引推荐

| 索引 | 特点 | 推荐场景 | 起始参数 |
|---|---|---|---|
| HNSW | 高召回、低查询延迟、内存较高、构建较慢 | 常见中等规模 RAG 默认 | `M=16..32`, `efConstruction=128..256`, search `ef=64..256` |
| IVF_FLAT | 内存较可控、参数直观、需训练 | 更大规模且可接受调参 | `nlist` 从约 `4*sqrt(N)` 附近压测，`nprobe` 从 nlist 的 1% 至 10% 测试 |
| IVF_SQ8 | 节省内存，有量化损失 | 成本优先且可接受召回下降 | 继承 IVF 调参并做质量对照 |
| DISKANN | 使用磁盘换内存，适合很大数据 | 大规模、NVMe、版本能力验证后 | 按官方版本和硬件基准配置 |
| AUTOINDEX | 降低选择成本 | 托管服务或快速起步 | 仍需记录实际索引和压测结果 |
...
loaded_memory_budget =
    raw_vector_bytes * measured_index_factor * loaded_replicas
```

月度成本至少拆成：

- Document/Query Embedding 与 Rerank 调用；
- Milvus 计算节点、已加载内存和索引构建节点；
- 对象存储、备份、跨区复制和网络流量；
...
- 重建按 tenant/knowledge base 分片，限并发、可暂停、可续传。
...
| Milvus 不可用 | 使用 PostgreSQL FTS；若当前问题强制知识模式且关键词无结果，返回可重试错误 |
...
|---|---|---|---|---|---|---|---|
| Milvus Standalone | 完整向量能力，单节点上限受资源约束 | 纵向为主 | 中 | 低至中 | 最终一致双库 | 标量隔离，HA 弱 | MVP、测试、可停机的小生产 |
| Milvus Distributed | 高吞吐、独立查询/写入/索引扩展 | 高 | 高 | 中至高 | 最终一致双库 | partition/resource group/专属 placement | 高可用、大规模、长期推荐 |
...
| Milvus Lite | 本地嵌入、启动快 | 低 | 很低 | 很低 | 开发工具 | 非生产隔离 | 原型和算法实验 |
| 托管向量数据库 | 能力和弹性取决于产品，通常开箱即用 | 高 | 低 | 持续服务费较高 | 最终一致双库 | 通常提供逻辑/物理隔离 | 团队运维能力有限、需要快速生产化 |

结合本项目：

- 已决定资源中心独立使用 Milvus，因此 pgvector 不作为目标主路径，但仍可服务现有模块和 PostgreSQL 全文兜底。
- 第一阶段用 Standalone 验证业务闭环。
- 生产需要节点故障自动恢复、滚动升级或持续扩容时，选择 Distributed 或托管服务。
...
- 向量达到数百万、峰值 QPS 达到两位数、索引重建明显干扰在线查询，或业务要求 HA 时，启动 Distributed/托管评估。
- 数据小、QPS 低且无 HA 要求时，Standalone 可以继续，但必须有备份和重建演练。
...
| Collection 爆炸 | 控制面和内存浪费 | 场景/模型共享，禁止按用户创建 |
...
| Milvus 单点 | RAG 不可用 | 生产 Distributed/托管，PG FTS 降级 |
| 运维能力不足 | 恢复和升级失败 | 托管选项、Runbook、季度恢复演练 |
| 过早拆微服务 | 交付慢、契约膨胀 | 先模块化端口 + 独立 worker 进程 |
| 同时维护 pgvector | 三份状态、对账复杂 | 资源中心向量只写 Milvus |
...
- 增加 Milvus adapter、Standalone Compose profile 和 `cmd/vector-worker`。
...
- Milvus 故障时 PostgreSQL FTS 降级。

涉及：Resource application、Session application、PostgreSQL、对象存储、Embedding adapter、Milvus、worker。
...
- 增加 PostgreSQL FTS + Milvus 的 RRF 混合检索和 rerank。
- 接入 Prometheus、trace、结构化日志、检索审计和质量看板。
- 建立备份 epoch、恢复、全量重建和升级 Runbook。
- 依据 SLA 选择 Milvus Distributed 或托管服务。
...
主要风险：集群运维复杂、对象存储/etcd 依赖、外部模型数据合规。

验收标准：

- 任一 API/worker 实例重启不丢任务；
- Milvus 节点故障不造成超出 SLO 的整体中断，或自动降级；
- dead 和对账差异可告警、dry-run、补偿；
- 完成一次 PostgreSQL + 对象存储 + Milvus 协调恢复演练；
- 权限、删除和租户边界通过专门安全验收。
...
- 基于生产分布对 HNSW、IVF、DISKANN/AUTOINDEX 做基准。
- 优化 partition key、segment、加载副本、批量写入和重建限流。
- 建立 query/embedding 缓存、热知识库预加载和邻接 chunk 读取。
- 调整 chunk、overlap、TopK、RRF、rerank 和上下文预算。
- 推行模型/切片/索引变更的离线和灰度评测。

涉及：Milvus、Embedding/Rerank、PostgreSQL 查询、缓存、质量平台。
...
- 对超大/监管租户提供专属 Milvus Database/Collection/resource group。
- 跨可用区、跨区域灾备和自动故障演练。
- 支持多语言路由、父子检索、query expansion、自适应 TopK 和多模态 chunk。
- 建立新模型双索引、影子检索、自动质量回归和受控切换。

涉及：身份权限、资源中心、Milvus 集群、模型平台、灾备平台。
...
| 备份 | RPO、RTO、保留期、跨区要求 |
| 部署 | Standalone、Distributed 或托管的阶段门槛 |

## 19. 官方参考

- [Milvus 文档](https://milvus.io/docs)
- [Milvus 多租户](https://milvus.io/docs/multi_tenancy.md)
- [Milvus 一致性](https://milvus.io/docs/consistency.md)
- [Milvus Upsert](https://milvus.io/docs/upsert-entities.md)
- [Milvus 向量索引](https://milvus.io/docs/index-explained.md)
- [Milvus Standalone Docker Compose](https://milvus.io/docs/install_standalone-docker-compose.md)
- [Milvus Kubernetes/Helm](https://milvus.io/docs/install_cluster-helm.md)
- [Milvus Backup](https://milvus.io/docs/backup_overview.md)
...
具体 API、参数可用性和部署依赖必须以项目最终锁定的 Milvus release 官方文档为准，并通过集成测试验证。
