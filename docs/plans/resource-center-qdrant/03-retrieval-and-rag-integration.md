...
> 状态：`DONE`（13/13 开发项及 M3 测试环境验收全部通过）
> 里程碑：M3 MVP 可用
> 完整验收前置依赖：[P2-B 入库与向量索引](02-ingestion-and-vector-indexing.md) `DONE`；M2 入库闭环已完成
> 后续阶段：[P4 生产就绪](04-production-readiness.md)
> 模型配置：查询 embedding 使用管理员激活的不可变模型版本；可选 rerank 使用管理员的 `resource_reranker` Agent 配置。代码、环境变量和请求参数不得另选模型。
> 当前执行：全部开发项和 P2-B 入库已实现；2026-09-06 已获真实测试环境评测授权，固定语料与门槛见[测试环境验收](TEST-ACCEPTANCE-2026-09-06.md)。真实质量、一致性和 10 万目标容量通过，百万扩展趋势完成；后续明确授权的 5 次真实 Tutor 无答案问答及完整答案核验通过，M3 为 `DONE`。
...
| 字段 | 内容 |
|---|---|
| 状态 | `DONE`，13/13 开发项、P2-B、真实质量、目标容量及真实 Tutor 无答案问答验收全部通过 |
| 负责人 | Codex |
| 开始日期 | 2026-09-05 |
| 完成日期 | 开发项及阶段验收均为 2026-09-06 |
...
| 验证结果 | 8.2/8.3 为先前开发交付的隔离与 Mock 验证；最新真实模型、业务入库、四格式、故障修复与容量记录见 8.4 和测试环境验收，不能继续按“无外部模型调用”描述本轮整体 |
...
| 遗留风险 | 当前 P3 MVP 无未完成开发或验收项。百万档出现 4%-5% 降级，其调优属 P5，不宣称生产 SLO 达标；原创语料的真实模型质量及 5 条无答案样本是工程基准，不等同真实师生教学效果评估 |
...
### 8.3 提交前开发交付验收（2026-09-06，继续 P2-B 前的历史证据）
...
- 当次验收仅允许提交开发成果，未改变当时的 P2-B、D-004/D-005 和 M3 未通过状态；当次未执行业务数据库迁移、外部模型调用或远程发布。此历史边界不代表后续已授权的 P2-B 与真实评测仍暂停。

### 8.4 P2-B 后的真实质量与一致性

- 最新有效证据统一记录于[2026-09-06 测试环境验收](TEST-ACCEPTANCE-2026-09-06.md)：60 篇原创数学讲义、100 条预标注查询，质量与 10 万容量阈值在评测结果产生前冻结。外部模型只接收已授权原创资料，容量副本复用真实文档向量。
- 首轮真实文档入库约 35.7 秒，60 篇均发布。最终生产代码复测 Recall@5/MRR@5/nDCG@5 均为 1.0，500 条引用身份及原文摘要核对通过，完整检索 P50/P95/P99 为 839.026/1267.908/1630.055 ms，降级为 0；相同请求 ID 重放没有重复入库。
- PDF/DOCX/TXT/MD 均通过上传、解析、embedding、发布、检索、引用后删除；缺 1/错 1/多 1 的真实 PG/Qdrant 对账排修复 2、删除 1，复核差异为 0。完整重建前旧代仍可检索，切换后旧引用拒绝，旧代保留期内向量未被误删。
- P2 新增 0020/0021 迁移接入任务、outbox、暂存和 30 天终态清理；摘要保留不改变当前用户、资源、版本、generation 或引用授权。维护工具默认 dry-run，自动对账只扫描 PG 已登记代际。
- 10 万条/5 并发的本地与完整检索 P95 为 427.064/622.075 ms，达到冻结门槛；百万档分别为 4679.479/4612.032 ms，并有 5/100、2/50 降级，只记录扩展趋势。999940 个副本已清理，当前代恢复 60 点且对账差异为 0；旧代到期后 60 点实际清除。
- 容量扩展包含显式向量副本，不能作为百万独立文档的真实语义质量证据。显式拒绝权限负例已通过；后续明确授权的 5 次真实 Tutor 无答案问答全部成功，逐条完整回答无错误资料引用或来源冒认，M3 验收通过。
- 无答案补验沿用原 5 个问题，使用重新冻结的无关原创候选和内存消息存储替身，经正式 Session 上下文及真实 Tutor 执行；管理员配置数据库只读。每条上下文 2908 字节，25 条候选身份/hash 通过；不将此轮当作新的 PG/Qdrant 集成验证或 5/5 拒答。输入指纹、逐题结果与边界见本轮验收记录。
